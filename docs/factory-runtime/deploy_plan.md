...
This iteration keeps the GitHub Pages operator workspace render layer contract centered on selected-thread switch continuity. The bounded expectation is that intentional thread switches keep the shell mounted, clear stale old-thread ownership immediately, and show exactly one compact `SWITCHING` placeholder until the new snapshot attaches.
...
4. Intentionally switch between conversations and confirm the old thread loses live and follow ownership immediately.
5. Confirm the transcript shell and footer composer remain mounted during the switch instead of flashing a generic empty reset.
