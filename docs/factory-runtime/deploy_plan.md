...
Iteration 338 deploy gate expectation: when a thread is selected, restore, reconnect, and polling-fallback truth now remain legible from the center timeline session block plus the fixed composer surfaces, while secondary autonomy and execution cards stay suppressed and no degraded selected-thread fact is split across both the center lane and the side detail cards.
Iteration 337 deploy gate expectation: healthy selected-thread inline and footer session surfaces now show `REVIEW n/2`, `VERIFY n/2`, and `READY` only from the selected-thread SSE-owned `session_status` path, and reconnect, switch, deselection, or polling fallback immediately clear those quorum and ready markers instead of preserving the last healthy values.
Iteration 339 deploy gate expectation: healthy selected-thread runs now keep the center pane on one canonical inline session item with duplicate SSE session-event rows suppressed, and provisional selected-thread attach or resume uses that same collapsed session item contract until authority degrades or clears.
Iteration 340 deploy gate expectation: selected-thread send and first-response handoff now stay on that same canonical inline session item, with pending-user and pending-assistant placeholders suppressed behind it and composer-adjacent handoff copy cleared until the first assistant append or an explicit downgrade changes the lane.
Iteration 344 deploy gate expectation: when the selected thread owns a healthy SSE session, the footer strip must carry the same blocker, intended-path, verifier, quorum, and ready metadata as the canonical center session block, and degraded or fallback paths must clear those footer autonomy datasets immediately while the secondary autonomy card remains suppressed only for the healthy lane.
Iteration 343 deploy gate expectation: during intentional thread switching the sticky active-session rail must stay hidden, the center placeholder plus bottom composer must carry the switch state, and reconnect or polling fallback must remain visibly distinct by using degraded rail cues instead of reusing the SWITCHING lane.
