...
Keep the selected-thread workspace mounted during intentional thread switches.

1. Keep the change bounded to selected-thread switch continuity and matching verifier expectations.
2. Preserve the existing selected-thread `session_status` plus SSE authority path.
3. Keep the selected-thread shell mounted while an intentional switch is waiting for the incoming snapshot.
4. Render one compact canonical switch placeholder in the center timeline.
5. Keep the bottom-fixed composer dock mounted and clear old-thread ownership immediately.
6. Block generic empty-state fallback during active switch transitions.
7. Align proposal artifacts with the already-present mounted-switch contract.
