...
Iteration 338 keeps transport, polling fallback, and selected-session authority unchanged and narrows only the non-healthy presentation boundary: restore, reconnect, and polling-fallback selected-thread state now render through the primary center timeline session block while secondary autonomy and execution cards stay suppressed whenever the selected session is already represented in the main workspace.
Iteration 337 keeps transport, switching, polling fallback, and session ownership unchanged and narrows only the quorum-authority seam: review quorum, verify quorum, and ready progression now derive from one selected-thread `session_status` helper, and non-authoritative paths clear or downgrade those cues immediately instead of carrying forward stale healthy counts.
Iteration 339 keeps transport, degraded fallback, and selected-session authority unchanged and narrows only the center-timeline duplication seam: healthy and provisional selected-thread progress now mutate in one canonical inline session item while duplicate SSE session-event rows are suppressed until selected-thread authority degrades or clears.
Iteration 340 keeps transport, degraded fallback, and selected-session authority unchanged and narrows only the submit-to-first-append handoff seam: selected-thread send and pending-assistant progression now mutate in the canonical inline session item while pending placeholders and composer-adjacent handoff copy no longer compete as parallel live-status surfaces.
Iteration 344 keeps transport and fallback behavior unchanged and narrows only the healthy autonomy-presentation seam: blocker, intended-path, verifier, quorum, and ready state now project through the same healthy center session block and footer strip, while the separate secondary autonomy detail surface remains suppressed whenever that healthy selected-thread session already owns the workspace.
Iteration 343 keeps transport and selected-session authority unchanged and narrows only the switch-versus-fallback presentation boundary: intentional thread switching now clears the sticky active-session rail entirely and leaves switching visible only in the center selected-session lane plus bottom composer, while degraded reconnect and polling fallback remain the only rail-owned exception paths.
