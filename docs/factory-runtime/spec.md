...
Iteration 340: selected-thread submit and first-response handoff now collapse into that same canonical inline session item in this branch. Pending-user send, accepted, and generating progression mutate in place there, while pending placeholders and composer-adjacent handoff chrome stay suppressed until the first assistant append or an explicit downgrade clears ownership.
Iteration 344: healthy selected-thread autonomy progress now lives on one session-owned lane in this branch. The canonical center session block and bottom strip both carry blocker, intended-path, verifier, quorum, and ready state during healthy SSE ownership, while the secondary autonomy card stays suppressed until ownership degrades or leaves the healthy lane.
Iteration 343: intentional selected-thread switching no longer reuses the sticky active-session rail in this branch. Switch ownership now stays only in the center timeline placeholder plus bottom composer state, while the sticky rail remains reserved for restore, reconnect, and polling-fallback explanation cues.
