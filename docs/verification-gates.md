...
- the selected conversation should expose at most one compact composer-adjacent live strip above the active composer, sourced only from the current selected-thread SSE-owned run, and that strip should disappear on reconnect downgrade, terminal, or polling-only fallback so it does not duplicate the transcript-tail contract
...
- the selected-thread center header should expose one compact machine-readable live-session ownership indicator adjacent to that summary row, showing `SSE OWNER` only while the selected thread remains healthy and SSE-owned, downgrading to `RECONNECT` or `POLLING` on retry or session rotation, and clearing on thread switch or terminal completion
