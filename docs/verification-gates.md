...
- the selected transcript should stay pinned to the newest append only while the user is already near the bottom, a compact jump-to-latest control should appear only after unseen live content arrives off-screen, and using that control or re-engaging the active composer should restore follow mode
...
- after composer submit, the selected transcript should publish one machine-readable handoff stage with either exactly one pending outbound user turn or exactly one temporary assistant placeholder, never both at once, until the first real assistant SSE append or a clear condition resolves it
