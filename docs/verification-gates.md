...
- the footer should read as a compact chat-style composer with the textarea as the dominant input surface, send as the primary action, and proposal apply plus auto-open compressed into a compact secondary utility cluster so message entry no longer looks like an admin form
