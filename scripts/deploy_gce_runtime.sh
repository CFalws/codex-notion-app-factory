...
REMOTE_CMD="sudo -u ${GCE_APP_USER} git config --global --add safe.directory ${GCE_REPO_PATH} && sudo -u ${GCE_APP_USER} git -C ${GCE_REPO_PATH} pull --ff-only origin main && sudo systemctl restart ${GCE_SERVICE_NAME} && sudo systemctl is-active ${GCE_SERVICE_NAME} && sudo -u ${GCE_APP_USER} git -C ${GCE_REPO_PATH} rev-parse HEAD"
