...
echo "$apps" | grep -q "\"app_id\":\"$APP_ID\""
...
required={"conversation.created","message.accepted","intent.interpreted","job.queued","job.running","job.completed"}
...
unexpected=sorted(forbidden & set(events))
if unexpected:
