...
def assert_console_contract(ops_url: str, api_key: str) -> None:
    ...
    require(render_js, 'label: followPaused ? "FOLLOW PAUSED" : "FOLLOWING"', label="live-follow healthy label")
    ...
    require(render_js, 'const stage = handoffVisible ? "handoff" : String(liveRun.state || "live");', label="inline block handoff stage mapping")
    ...
    require(render_js, 'const pausedVisible = liveOwned && !Boolean(liveFollow.isFollowing);', label="follow control paused visibility guard")
    require(render_js, 'const followState = unseenCount > 0 ? "new" : "paused";', label="follow control state mapping")
    require(render_js, 'const isVisible = Boolean(liveOwned && (pausedVisible || (liveFollow.jumpVisible && unseenCount > 0)));', label="follow control visibility mapping")
