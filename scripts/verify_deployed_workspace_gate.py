...
() => {
  ...
  return {
    ...
    conversationMeta: conversationMeta ? {
      hidden: !!conversationMeta.hidden,
      text: (conversationMeta.textContent || "").trim(),
    } : null,
            ...
            page.wait_for_function(
                """conversationId => {
                  ...
                  return Boolean(
                    ...
                    threadPhase.dataset.selectedSessionState === "cleared" &&
                    threadPhase.dataset.selectedSessionOwned === "false" &&
                    threadPhase.dataset.selectedSessionTransport === "SNAPSHOT" &&
            ...
            page.wait_for_function(
                """() => {
                  ...
                  return (
                    ...
                    selectedCardLiveOwnerRow.dataset.liveOwnerEmphasis === "snapshot" &&
...
def assert_console_contract(ops_url: str, api_key: str) -> None:
    ...
    require(render_js, 'if ((item.pending_assistant || item.pending_local) && inlineState.handoffVisible) {', label="pending local or assistant placeholder suppression behind inline block")
    ...
    require(render_js, 'dom.threadPhaseChip.hidden = !phaseBadgeVisible;', label="healthy header badge visibility")
    ...
    require(render_js, "const sessionStatusLiveRun = deriveSelectedThreadHealthyLiveRunModel(currentState, conversation);", label="healthy selected-thread session-status live-run source")
    require(render_js, "if (sessionStatusLiveRun.visible) {", label="healthy selected-thread session-status live-run promotion")
    require(render_js, 'state: sessionStatusLiveRun.state,', label="healthy live-run state from session-status")
    require(render_js, 'phase: sessionStatusLiveRun.phase,', label="healthy live-run phase from session-status")
    require(render_js, 'detail: sessionStatusLiveRun.detail,', label="healthy live-run detail from session-status")
    require(render_js, 'source: sessionStatusLiveRun.source,', label="healthy live-run source from session-status")
    require(render_js, 'terminal: sessionStatusLiveRun.terminal,', label="healthy live-run terminal from session-status")
    require(store_js, "export function deriveSelectedThreadHealthyLiveRunModel", label="healthy session-status live-run helper")
    require(store_js, 'payloadConversationId !== conversationId', label="healthy live-run selected-thread conversation match guard")
    require(store_js, 'proposalReady || proposalStatus === "ready_to_apply" || rawPhase === "READY"', label="healthy live-run ready authority from session-status")
    require(store_js, 'proposalStatus === "applied" || rawPhase === "APPLIED"', label="healthy live-run applied authority from session-status")
    require(store_js, 'rawPhase === "FAILED" || pathVerdict === "DEGRADED" || blockerReason !== "none"', label="healthy live-run degraded failure authority from session-status")
    ...
    require(render_js, 'dom.threadPhaseChip.dataset.selectedSessionState = phaseBadgeVisible ? sessionSnapshot.state : "cleared";', label="header phase badge selected-session state dataset")
    require(render_js, 'dom.threadPhaseChip.dataset.selectedSessionTransport = phaseBadgeVisible ? sessionSnapshot.transportLabel : "SNAPSHOT";', label="header phase badge selected-session transport dataset")
    require(render_js, 'dom.threadPhaseChip.dataset.selectedSessionReason = phaseBadgeVisible ? sessionSnapshot.reason : "idle";', label="header phase badge selected-session reason dataset")
    require(render_js, 'dom.threadPhaseChip.dataset.selectedSessionPhase = phaseBadgeVisible ? sessionSnapshot.phaseLabel : "IDLE";', label="header phase badge selected-session phase dataset")
    require(render_js, 'dom.conversationMeta.hidden = metaText.length === 0;', label="header meta hidden when empty")
    ...
    require(render_js, 'visible: handoffVisible || liveVisible || degradedVisible || provisionalVisible || restoreVisible,', label="inline block unified visibility contract")
    ...
    require(render_js, 'Boolean(dom.sessionStrip && !dom.sessionStrip.hidden && owner.conversationId) || Boolean(inlineState.handoffVisible);', label="composer owner merged strip or inline-handoff guard")
    ...
    require(render_js, 'const handoffPhaseLabel = handoffVisible', label="inline handoff phase mutation helper")
    require(render_js, 'data-live-block-handoff-state="${escapeHtml(handoffVisible ? handoffStage : "idle")}"', label="inline handoff stage dataset")
    ...
    require(render_js, 'data-live-block-canonical="${primarySessionOwned || handoffVisible ? "true" : "false"}"', label="inline canonical live session dataset")
    require(render_js, 'data-live-block-duplicates="${collapseSessionEvents || handoffVisible ? "collapsed" : "allowed"}"', label="inline session-event collapse dataset")
    ...
    require(conversations_js, 'card.dataset.liveOwnerEmphasis = "snapshot";', label="selected live owner emphasis reset")
    ...
    require(conversations_js, 'card.dataset.liveOwnerEmphasis = shadowSelectedRowLiveMarker ? "strong" : "soft";', label="selected row card emphasis dataset")
    ...
    require(conversations_js, 'liveOwnerRow.dataset.liveOwnerEmphasis = showSelectedRowLiveMarker && shadowSelectedRowLiveMarker ? "strong" : "soft";', label="selected row owner row emphasis dataset")
    ...
    require(styles_css, '.conversation-card.active[data-live-owner="true"][data-live-owner-emphasis="strong"]', label="selected row strong live owner card styling")
    require(styles_css, '.conversation-card-live-owner-row[data-live-owner-emphasis="strong"]', label="selected row strong owner row styling")
    ...
    require(store_js, 'clearReason: "thread-switch",', label="active session switching clear reason")
    ...
    require(store_js, "(sessionStatus.pendingHandoff && sessionStatus.selectedThreadSse)", label="selected-thread handoff ownership suppression")
    require(store_js, "provisionalOwned ||", label="selected-thread session ownership provisional-or-healthy gate")
    require(store_js, "healthyPromotion.promoted &&", label="selected-thread session ownership healthy promotion gate")
